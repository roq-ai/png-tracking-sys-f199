...
import { getManagers } from 'apiSdk/managers';
...
function DashboardPage() {
  ...
  const analystStat = useSWR<PaginatedInterface<AnalystInterface>>(
    () => `/analysts`,
    () => getAnalysts({ limit: 1, offset: 0 }),
  );
  ...
  const managerStat = useSWR<PaginatedInterface<ManagerInterface>>(
    () => `/managers`,
    () => getManagers({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Analysts',
      result: analystStat?.data?.totalCount,
      isLoading: analystStat.isLoading,
    ...
    {
      title: 'Managers',
      result: managerStat?.data?.totalCount,
      isLoading: managerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
